// File: src/memory/cold_tier.cpp
//
// Cold Tier Implementation (File-based storage for HDD)
//
// Uses simple file-based storage optimized for HDD access patterns.
// Target latency: <1ms
...
class ColdTier : public IMemoryTier {
public:
    explicit ColdTier(const std::string& storage_path)
    ...
    void Compact() override {
        // Could implement file consolidation here
    }
...
std::unique_ptr<IMemoryTier> CreateColdTier(const std::string& storage_path) {
    return std::make_unique<ColdTier>(storage_path);
}
